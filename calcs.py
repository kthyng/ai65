...
kind = 'others_new' #'barotropic'  # 'baroclinic'
...
for i in range(ntimes):
    ...
    # PEnorth[i] = rho0 * g * dx * (Hnorth**2 * vbarnorth).sum()  # potential energy
    # # PEnorth[i] = rho0 * g * dx * (Hnorth * vbarnorth * zetanorth).sum()  # potential energy anomaly
    # if kind == 'barotropic':
    ...
    # PEsouth[i] = rho0 * g * dx * (Hsouth**2 * vbarsouth).sum()  # potential energy
    # # PEsouth[i] = rho0 * g * dx * (Hsouth * vbarsouth * zetasouth).sum()  # potential energy anomaly
    # if kind == 'barotropic':
